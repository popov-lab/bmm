test_that("add_subset() attaches subset() with the correct separator", {
  ...
  two_aterms <- add_subset(brms::bf(rt | dec(resp) + trials(n) ~ x), "s3")
  expect_equal(deparse1(two_aterms$formula), "rt | dec(resp) + trials(n) + subset(s3) ~ x")
})
...
test_that("a multivariate specification requires at least two components", {
  expect_error(
    configure_fit(new_mvbmmformula(list(mv_comp_mixture))),
    "at least two components"
  )
})

test_that("models with Stan code outside the functions block are rejected", {
  joint <- bmm_component(bmf(c ~ 1, kappa ~ 1), model = sdm(resp_error = "error"), data = mv_dat_vwm) +
    mv_comp_lognormal
  expect_error(stancode(joint), "injects Stan code into")
})
...
test_that("response variables must be unique after brms name sanitization", {
  ...
  expect_error(stancode(joint), "must be unique across the components")
})
...
test_that("data columns clashing with subset indicators are rejected", {
  ...
  expect_error(stancode(joint), "conflict with the subset indicators")
})
...
test_that("extractors skip the init function", {
  cfg <- suppressMessages(configure_fit(mv_comp_ddm + mv_comp_mixture, init = FALSE))
  expect_null(cfg$config_args$init)
})
...
test_that("components with init_ranges produce a valid init function", {
  ...
  cfg <- suppressMessages(configure_fit(joint))
  initfun <- cfg$config_args$init
  expect_true(is.function(initfun))

  inits <- initfun()
  scode <- suppressMessages(stancode(joint))
  spars <- names(extract_parameter_dimensions(extract_stan_blocks(scode)$parameters))
  expect_true(all(names(inits) %in% spars))

  ranges <- .model_ddm()$init_ranges
...
test_that("the init list covers every parameter of the Stan model", {
  ...
  cfg <- suppressMessages(configure_fit(joint))
  spars <- names(extract_parameter_dimensions(
    extract_stan_blocks(suppressMessages(stancode(joint)))$parameters
  ))
