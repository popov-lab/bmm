...
test_that(".gwald handles nonpositive x and is continuous around drift = 0", {
  expect_identical(bmm:::.gwald(c(-0.2, 0), 2, 1, 1), c(-0.2, 0))
  ...
  for (drift in c(0, 1e-7, -1e-7, 1e-5, -1e-5)) {
    g_num <- stats::integrate(
      function(u) {
        bmm:::.pwald(u, drift, 1.2, s = 1, lower.tail = FALSE, log.p = FALSE)
      },
      lower = 0, upper = 0.5, rel.tol = 1e-12
    )$value
    expect_equal(bmm:::.gwald(0.5, drift, 1.2, 1), g_num, tolerance = 1e-7)
  }
})
