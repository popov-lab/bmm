...
.sdt_metad_stanvars <- function(model) {
  ...
  stan_metad   <- read_lines2(file.path(sc_path, "sdt_metad_funs.stan"))
  stan_wrapper <- .sdt_rating_stan_wrapper(model, "sdt_metad", "sdt_metad_row_lpmf")
  brms::stanvar(
    scode = paste(stan_binary, stan_rating, stan_metad, stan_wrapper, sep = "\n"),
    block = "functions"
  )
}
...
configure_model.sdt_metad <- function(model, data, formula) {
  ...
  stanvars <- .sdt_metad_stanvars(model)
