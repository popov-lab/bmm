...
print.bmm_component <- function(x, ...) {
  print_model_header(x$model, x$formula, x$data_name %||% "", nrow(x$data), links = FALSE)
  if (!is.null(x$prior)) {
...
validate_component_family <- function(family, formula) {
  ...
  stopif(
    family$family %in% c(
      "binomial", "beta_binomial", "zero_inflated_binomial",
      "zero_inflated_beta_binomial", "multinomial", "dirichlet_multinomial"
    ),
