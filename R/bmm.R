...
#'   model to be fitted. Alternatively, a multivariate specification created by
#'   combining [bmm_component()] objects with `+`, in which case the `data` and
#'   `model` arguments must be omitted (each component carries its own).
...
#'
#'   # Multivariate models (experimental)
#'
#'   Several models can be estimated jointly to obtain the correlations of
#'   their subject-level parameters. Build a component for each model with
#'   [bmm_component()], combine the components with `+`, and pass the result
#'   as the `formula` argument (omitting `data` and `model`). See
#'   [bmm_component()] for details.
#'
#'   # Miscellaneous
#'
...
bmm <- function(formula, data, model,
  ...
  if (is_mvbmmformula(formula)) {
    ...
    return(bmm_mv(formula,
      prior = prior, sort_data = sort_data, silent = silent,
      backend = backend, file = file, file_compress = file_compress,
