...
#' @param prior Optional `brmsprior` object used for model fitting. Needed when
#'   constant priors reduce free parameters in the Stan code.
#' @param stanvars Optional `stanvars` object passed to brms.
#'
...
create_initfun <- function(model, data, formula, prior = NULL, stanvars = NULL) {
  UseMethod("create_initfun")
}

#' @export
create_initfun.bmmodel <- function(model, data, formula, prior = NULL, stanvars = NULL) {
  ...
  stan_code <- stancode(formula, data, formula$family, prior = prior, stanvars = stanvars)
...
init_real_param <- function(par, init_range, link) {
  ...
  } else if (grepl("^par_b_", par)) {
    link_transform(runif(1, min = init_range[1], max = init_range[2]), link)
  } else if (grepl("^par_sd_", par)) {
    runif(1, min = 0.05, max = 0.1)
  } else if (grepl("sd_", par)) {
    runif(1, min = 0.05, max = 0.1)
  } else {
    stop2("Initial values for real parameters are only specified for intercepts, regression coefficients, and sd-parameters")
  }
}
