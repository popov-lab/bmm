...
mvbmm_config <- function(spec, prior = NULL) {
  ...
  init <- mv_create_initfun(components, formula, data)
...
# the shared random-effects and correlation parameters generically; all other
# parameters keep the sampler's default inits (partial init lists are
# accepted by all backends)
mv_create_initfun <- function(components, formula, data) {
  needs_init <- vapply(
    components,
  ...
  standata_list <- standata(formula, data)
  stan_code <- stancode(formula, data)
  ...
  function() {
    ...
    for (spar in names(stanpars_list)) {
      ...
      dim <- unlist(standata_list[stanpars_list[[spar]]$dims])
