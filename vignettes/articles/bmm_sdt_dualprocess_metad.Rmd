...
Gaussian comparison in the core article; @MeyerGrant_2026). The choice is best
...
metacognitive one. `d` and meta-d′ are then both $d_a$ indices, so the M-ratio
keeps its meaning. Second, the model recovers known M-ratios from simulated
...
```{r cdp-estimates, include = FALSE}
fmt2 <- function(x) format(round(x, 2), nsmall = 2)
cdp_est <- brms::fixef(fit_cdp)[, "Estimate"]
```

Recollection (`drec` ≈ `r fmt2(cdp_est[["drec_Intercept"]])`) and familiarity
(`dfam` ≈ `r fmt2(cdp_est[["dfam_Intercept"]])`) contribute comparable
sensitivities, the Remember criterion sits well into the recollection
distribution (`rcrit` ≈ `r fmt2(cdp_est[["rcrit_Intercept"]])`), and
`exp(sigmar)` ≈ `r fmt2(exp(cdp_est[["sigmar_Intercept"]]))` puts the
...
- Stay with **`version = "standard"`** when a single strength process suffices;
  ...
  (`Ro`, `Rn` → 0; `logmratio` = 0, i.e. meta-d′ = `d`).
...
between accounts that can mimic one another [@MeyerGrant_2026; @Robinson_2023].
