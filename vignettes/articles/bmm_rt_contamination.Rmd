...
If a cell's observed accuracy is higher than the estimated contamination and guess rate can produce — 95% correct is impossible when 15% of the trials were coin flips — the split cannot be trusted for that cell. The correction still runs, removing the estimated contaminants proportionally from both `n_upper` and `n_trials` and leaving observed accuracy as it was, but it warns. That usually means the guess rate does not match the task, or that the contaminant reaction times were not distinguishable enough from the cognitive ones for the EM to separate them.
