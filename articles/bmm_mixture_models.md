...
([`?mixture3p`](https://venpopov.com/bmm/reference/mixture3p.md)) adds a
third state: confusing the cued object with another object shown during
encoding and thus reporting the feature of the other object (long dashed
green distribution in Figure [1.1](#fig:mixture-models)). Responses from
this state are sometimes called non-target responses or swap errors. The
non-target responses also come from a von Mises distribution centered on
the feature of the non-target object. The probability of non-target
responses is represented by the parameter \\p\_{nt}\\, and the complete
model is:
...
[`?mixture2p`](https://venpopov.com/bmm/reference/mixture2p.md) and
...
[`bmm()`](https://venpopov.com/bmm/reference/bmm.md) function. The fit
model function uses the `brms` package to fit the model, so you can pass
to it any argument that you would pass to the `brm` function.
