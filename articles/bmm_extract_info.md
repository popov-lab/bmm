...
[`default_prior()`](https://venpopov.com/bmm/reference/default_prior.bmmformula.md)
from the `brms` package to extract the default priors for a model. The
arguments to `default_prior` are the same as for the
[`bmm()`](https://venpopov.com/bmm/reference/bmm.md) function in `bmm`.
For example, if you want to extract the default priors for the SDM model
([see the online article for more
...
the
[`stancode()`](https://venpopov.com/bmm/reference/stancode.bmmformula.md)
function from `brms`:
...
```
    ...
    // generated with brms 2.23.0 and bmm 1.3.2
    functions {
...
you can use the
[`standata()`](https://venpopov.com/bmm/reference/standata.bmmformula.md)
