...
### Bug fixes
* The initial values for `ndt` in the **cswald** model are now derived from the data instead of a fixed range. Previously `ndt` was initialized in `[0.025, 0.05]` seconds regardless of the response times supplied, so for data with very fast responses the initial value could exceed `min(rt)`, where the likelihood is `-Inf` and sampling cannot start. It is now drawn from `[0.4, 0.6] * min(rt)`, which is valid by construction. In a 30-fit simulation study this did not measurably change divergence counts or warmup time (`local/cswald_sampler_stability.R`); the change is about guaranteeing a usable starting point, not about sampling efficiency. Models can adapt their own initial value ranges to the data by adding a method for the new generic `adjust_init_ranges()` (#387).
* `check_data()` for the **cswald** model now warns when the default `ndt` prior places substantial mass above the fastest reaction time in the data, since the likelihood requires `ndt < rt` for every trial. The check compares the population-level prior against the overall minimum, so it is a floor rather than a guarantee when grouping factors are present (#387).
* The "crisk" version of the **cswald** likelihood now returns `-Inf` rather than `NaN` when an extreme `zr` collapses one of the two sub-boundaries to zero. Under the unbounded logit link `zr * bound` can underflow to 0, where `log()` is `-Inf` in exact arithmetic but `NaN` in floating point, and a `NaN` poisons the gradient of the entire proposal instead of rejecting it (#387).
* `swald_lpdf()` now returns `-Inf` for non-positive or infinite `bound` and `s` instead of silently producing `NaN` via `log()`. Unreachable with the default log links; relevant when the links are overridden with `identity` (#387).
...
* The **cswald** likelihood is hardened against two numerical failure modes near its boundaries. First, with `sndt` estimated, the density for response times in the strip `ndt < rt <= ndt + sndt` is now evaluated via the log-scale CDF (a stable log-sum-exp) instead of a survivor difference, which returned `-Inf` once the CDF underflowed (roughly `rt - ndt < bound^2 / (1416 * s^2)`) although the true log density is representable; the sampler now sees a finite value with a restoring gradient there instead of a gradient-free cliff. Second, all survivor-difference computations (`swald_lccdf` and its R mirror `log_diff_exp`) return `-Inf` (probability zero) instead of `NaN` when floating-point rounding makes the subtrahend exceed the minuend, which is reachable for extreme proposals when `s` is freed; a `NaN` there poisons the gradient of the entire proposal (#387).
