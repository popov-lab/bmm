...
* The **ezdm** likelihood no longer assumes reaction times are normally distributed. It treated the two RT summaries as independent, with `mean_rt` normal and `var_rt` scaled chi-square, which holds only for normal RTs. Diffusion decision times are right-skewed (kurtosis 8.8 at zero drift and higher away from it), so the sampling variance of `var_rt` was understated by a factor of 3.9 to 4.6 at ordinary parameter values, their correlation of about 0.65 was ignored, and posteriors came out too narrow. Both versions now use the exact first four cumulants of the first-passage time: `var_rt` follows a Gamma matched to the true mean and variance of a sample variance, and `mean_rt` is normal conditional on it, reproducing `Cov(mean_rt, var_rt)` exactly. The terms reduce to the previous ones when the third and fourth cumulants vanish. `rezdm()` samples from the same distribution and no longer truncates `mean_rt` at `ndt`, which `dezdm()` never assumed. See `?ezdm_dist` (#407).
* Fix `ezdm` moments returning `NaN` at large `|drift|`. The 4-parameter moments used `cosh(x)/sinh(x)`, which is `Inf/Inf` above `x = 710`, so every moment was `NaN` from roughly `drift * bound / s^2 > 1420`; the proportion correct used differences of `exp(2 k z)` and was `NaN` from about `|drift| > 400`. Both are now written in forms that stay finite, and the moments are computed from `drift^2 / s^2` throughout. Because they depend on the drift rate only through its square, the soft absolute value `sqrt(drift^2 + 0.01^2)` (which biased `drift^2` by `1e-4`) and the separate near-zero-drift formulas are both gone, removing a discontinuity in the gradient at `drift = 0` (#407).
