...
* `ezdm_summary_stats(method = "mixture")` now returns `n_upper` and `n_trials` for the responses its `mean_rt`/`var_rt` are based on. It previously returned the raw counts beside cleaned moments, so every cell told **ezdm** it rested on more responses than it had and its posterior came out too narrow. Summary statistics change and fits are not reproduced; recompute them and refit. Use `guess_rate` to set the accuracy expected of a contaminant response (`version = "3par"` only). `adjust_ezdm_accuracy()` is deprecated — applying it now removes the same contaminants twice (#423).
