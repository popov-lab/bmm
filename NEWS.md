...
### New models
* Add **Binary Signal Detection Theory** (`sdt_binary`) for yes/no detection and recognition tasks with aggregated hit and false-alarm counts. The model estimates sensitivity (`d`) and response bias (`criterion`) under a binomial likelihood, with a selectable noise distribution (`"normal"`, `"gumbel_min"`, `"gumbel_max"`, or `"logistic"`) and optional unequal-variance scaling via an `sdratio` parameter, which uses a log link and is fixed at 1 (equal variance) unless it is given a formula. Sensitivity is reported as *d_a*, the separation between the distributions divided by the root-mean-square of their standard deviations, which equals the familiar *d'* whenever `sdratio` is 1. Dividing by the noise SD alone is not comparable across conditions that differ in `sdratio`: two equally discriminable conditions can then show a large, confidently estimated sensitivity difference. `criterion` stays on the noise-standardized axis. Includes the distribution functions `dsdt_binary()` and `rsdt_binary()`, plus `sdt_d()` and `sdt_criterion()` to recover sensitivity and bias from observed hit and false-alarm rates. `dist` names the distribution of the latent evidence itself, under the usual decision rule "respond old when the evidence exceeds the criterion", and the `"gumbel_min"` / `"gumbel_max"` labels follow the extreme-value convention: `"gumbel_min"` is the smallest-extreme-value (complementary log-log) distribution and `"gumbel_max"` the largest (log-log, as in `evd::pgumbel`). Thanks to @GidonFrischkorn
