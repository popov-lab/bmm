...
### New features
* Add a **post-processing toolkit for signal detection models**: `roc_sdt()` computes the model-implied ROC curve from the posterior, `roc_observed()` the empirical ROC points from the data, and `auc_sdt()` the posterior area under the curve, each with `plot()` methods (`plot.bmm_sdt_roc()`, `plot.bmm_sdt_auc()`). These work for `sdt_binary` and `sdt_rating` (the models with a response criterion) and raise an informative error for the criterion-free `sdt_mafc` and `sdt_ranking`. For `sdt_binary`, when a predictor varies the criterion only (e.g. a base-rate manipulation, as in `broeder_schuetz_2009_e3`), `roc_sdt()` returns one smooth curve plus the model-implied operating points per criterion level — a curvilinear multi-criteria ROC. `plot.bmm_sdt_roc()` gains a `scale = "z"` argument that reads the rates on the noise distribution's quantile (probit-style) axis, on which the Gaussian/logistic model ROC is a straight line with slope `1 / exp(sdratio)` (a slope below 1 is the unequal-variance signature) and departures from linearity diagnose misfit. A new `latent_sdt()` extractor and `plot.bmm_sdt_latent()` method visualise the model-implied latent decision-variable distributions (noise vs. signal) with the criterion (binary) or confidence thresholds (rating) overlaid, including a credible band on the boundary locations. `pp_check()` for `sdt_rating` now defaults to faceting by `stimulus` so the signal and noise rating distributions are checked separately (pass `group = NA` to pool them). A new article, *Signal Detection Theory models*, documents the four SDT models and this toolkit. Thanks to @GidonFrischkorn
