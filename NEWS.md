...
### New models
* Add **Binary Signal Detection Theory** (`sdt_binary`) for yes/no detection and recognition tasks with aggregated hit and false-alarm counts. The model estimates sensitivity (`dprime`) and response bias (`criterion`) under a binomial likelihood, with a selectable noise distribution (`"normal"`, `"gumbel_min"`, `"gumbel_max"`, or `"logistic"`) and optional unequal-variance scaling via an `sdratio` parameter, which uses a log link and is fixed at 1 (equal variance) unless it is given a formula. Includes the distribution functions `dsdt_binary()` and `rsdt_binary()`, plus `sdt_dprime()` and `sdt_criterion()` to recover sensitivity and bias from observed hit and false-alarm rates. `dist` names the distribution of the latent evidence itself, under the usual decision rule "respond old when the evidence exceeds the criterion", and the `"gumbel_min"` / `"gumbel_max"` labels follow the extreme-value convention: `"gumbel_min"` is the smallest-extreme-value (complementary log-log) distribution and `"gumbel_max"` the largest (log-log, as in `evd::pgumbel`). Thanks to @GidonFrischkorn
* Add **m-Alternative Forced Choice Signal Detection Theory** (`sdt_mafc`) for accuracy-only m-AFC tasks where one of `m` alternatives contains the signal (DeCarlo, 2012). The model estimates sensitivity (`dprime`) under a binomial likelihood, with the same four selectable noise distributions as `sdt_binary`. The probability correct uses a closed-form softmax for `"gumbel_max"`, a closed-form Gamma ratio for `"gumbel_min"`, Gauss-Hermite quadrature for `"normal"`, and Gauss-Legendre quadrature for `"logistic"`. The set size `m` may be a constant or the name of a data column, so trials with different numbers of alternatives can be fit jointly. Includes the distribution functions `dsdt_mafc()` and `rsdt_mafc()`. Thanks to @GidonFrischkorn
