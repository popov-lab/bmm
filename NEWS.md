...
### New models
* Add **Multinomial Processing Tree models** (`mpt`) for tasks with categorical responses generated by discrete cognitive processing states. Models are specified as trees via `mpt_tree()`; equality constraints follow from shared parameter names, and the `restrictions` argument of `mpt()` and the importers accepts MPTinR/TreeBUGS-style equality and constant restrictions (`c("Dn = Do", "g = 0.5")`). Printing a model lists the trees, the restrictions and the identifiability bound (free parameters versus response categories minus 1 summed over trees); parameters fixed in the formula (`bmf(g = 0.5)`) are shown on the probability scale and mapped to the latent scale when the constant prior is built. The implementation supports multiple experimental conditions (trees), logit and probit links, design-fixed covariates inside branch expressions, simplex-constrained parameter groups via stick-breaking, non-linear (e.g., time-varying) parameter formulas, and response categories that are structurally impossible in some trees (`mpt_tree(impossible = )`). Trees are matched to observations through the `tree_id` argument, which names a data column holding one tree name per observation; effects of experimental conditions are specified in the parameter formulas, as for every other model. Includes distribution functions `dmpt()` and `rmpt()`, import functions `mpt_from_string()` (MPTinR-style model definitions) and `mpt_from_eqn()` (EQN files, with automatic sanitization of parameter names and a reported renaming map; restrictions are written with the EQN names), a `plot()` method that draws the processing-tree diagrams implied by the branch expressions, and an online article introducing the model (#255).
