...
### Bug fixes
* A name in a model's `links` argument that is not a parameter of that model is now an error instead of being added to the model: `sdm(resp_error = "y", links = list(kapa = "identity"))` reported a parameter `kapa` while `kappa` kept its default link. A name one edit from a parameter is read as that parameter, with a warning; a link allowing values the default excludes (e.g. `identity` for a positive parameter) also warns. **sdm**, **mixture2p**, **mixture3p** and **imm** refuse `links`: they build their likelihood from fixed links, so a link set there never reached the sampler (#420).
