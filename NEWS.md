...
### New features
* `mixture2p()`, `mixture3p()` and `imm()` now dispatch on the response arguments they are given: `resp_error` selects the continuous reproduction model and `response`/`probe`/`target` select the change detection model. `mixture2p_de()`/`mixture2p_cd()`, `mixture3p_de()`/`mixture3p_cd()` and `imm_de()`/`imm_cd()` are available to be explicit. Each constructor takes exactly one response format, and the two variants of each model have independent S3 classes so neither inherits the other's methods.
