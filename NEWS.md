...
### New models
* Add **Yes/No Signal Detection Theory** (`sdt_yn`) for detection and old/new recognition tasks with aggregated response counts. It estimates sensitivity (`d`) and response bias (`criterion`) under a binomial likelihood, with a selectable evidence distribution (`"normal"`, `"gumbel_min"`, `"gumbel_max"` or `"logistic"`). Give `sdratio` a formula to allow unequal variance; it is identified only when the criterion varies across conditions, and `d` is then *d_a* rather than the noise-standardized *d'*. Also adds `dsdt_yn()`, `rsdt_yn()`, `sdt_d()` and `sdt_criterion()`. Thanks to @GidonFrischkorn
