...
* Add **`mixture3p_cd()`** and **`imm_cd()`**, the three-parameter mixture model and the interference measurement model for single-probe change detection. Both share the decision rule described above and differ only in the retrieval distribution they supply, so the non-target structure enters through retrieval alone: the decision process treats non-target retrievals and guesses alike as uniform (Lin & Oberauer, 2022, Eq. 5 and B.20). That asymmetry is what produces the intrusion cost, whereby a probe matching a non-target is harder to reject than a new probe at the same distance from the target. For `imm_cd()` the target's mixture weight plays the role of P_s in their Equation B.21. Includes distribution functions `dmixture3p_cd()`, `rmixture3p_cd()`, `dimm_cd()` and `rimm_cd()` (#136).
* Add **`sdm_cd()`**, the signal discrimination model for single-probe change detection. The sdm is not one of the models Lin & Oberauer (2022) compare, so this extends their decision rule rather than reproducing a model they fit: the observer responds "change" when the memory density at the probe falls below the uniform scaled by the criterion, which is the same principle their Appendix B describes for the von Mises case. Because the sdm density is far more peaked than a von Mises, the integral over the decision arc uses an adaptive rule that splits at every periodic image of the density's peak and at multiples of the spike width before applying Gauss-Legendre, accumulating in log space. A fixed rule is not usable: 32 nodes are off by 149% and 128 nodes by 46%, where the adaptive rule holds to 4e-10 across the sampled parameter range. Includes distribution functions `dsdm_cd()` and `rsdm_cd()` (#136).
...
### New features
* `mixture2p()`, `mixture3p()`, `imm()` and `sdm()` now dispatch on the response arguments they are given: `resp_error` selects the continuous reproduction model and `response`/`probe`/`target` select the change detection model. `mixture2p_de()`/`mixture2p_cd()`, `mixture3p_de()`/`mixture3p_cd()`, `imm_de()`/`imm_cd()` and `sdm_de()`/`sdm_cd()` are available to be explicit. Each constructor takes exactly one response format, and the two variants of each model have independent S3 classes so neither inherits the other's methods.
