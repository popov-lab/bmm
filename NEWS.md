...
* Fix the Stan likelihood of the **cswald** model returning `NaN` for extreme reaction times at high drift rates. The survivor `swald_lccdf()` used `std_normal_lccdf(z)`, which underflows to `-Inf` for `z > ~8.3`; it now uses `std_normal_lcdf(-z)` and returns `-Inf` instead of `NaN` when rounding makes the survivor difference non-positive. Stan-side analog of #376 (#387).

### Other changes
* The Stan likelihood of the **cswald** model is now a vectorized (`loop = FALSE`) custom family instead of a per-observation loop, with both versions sharing one shifted-Wald density and survivor in `cswald_helper_functions.stan`. The survivor is computed in probability space through `Phi()`, whose derivative is exact, with the log-space form kept as a fallback in the deep tail. Under within-chain threading the family emits `dec[start:end]` so that each partial sum pairs its response slice with the matching decisions. The vectorized likelihood is verified against the scalar Stan form and against the R implementation used by `log_lik()` in the test suite (#387).
