...
* **Removed the machinery the circular mixture models needed to switch off components a trial's set size did not have**, now that their likelihoods index their own covariates: the `LureIdx*` indicator columns and the `inv_ss` column built by `check_data()`, the `constrain_set_size1_fixef()` / `constrain_set_size1_ranef()` constant priors, the `expS` helper parameter, and the corresponding entries in the internal-variable filter used by `conditional_effects()`.
* **`mixture3p` and `imm` no longer require the intercept to be suppressed when `set_size` is a predictor.** That restriction existed so the set-size-1 coefficient could be pinned by a constant prior; with a ragged likelihood there is nothing to pin, so `kappa ~ 1 + set_size` is now accepted.
