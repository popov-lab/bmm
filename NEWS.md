...
* `report_priors()` now names every kind of correlation prior the way `set_prior()` does, not only the one on group-level effects. A fit with two or more `me()` terms was reported as class `Lme` with `lkj_corr_cholesky(1)`; `set_prior()` does not accept that class, so the prior could not be overridden as the report wrote it. The same applied to `rescor`, `lncor` and `cortime`.
