...
* Fix `combine_prior()` erroring with `second argument must be a list` when its first argument is `NULL`. `set_default_prior()` returns `NULL` when `options(bmm.default_priors = FALSE)` is set, so that option made `bmm()` fail for every model instead of fitting with flat priors (#391).
...
* The **ezdm** likelihood no longer assumes reaction times are normally distributed. It treated the two RT summaries as independent, with `mean_rt` normal and `var_rt` scaled chi-square, which holds only for normal RTs. Diffusion decision times are right-skewed (with a symmetric start point their kurtosis is 8.8 at zero drift and falls towards 3 as drift grows), so the sampling variance of `var_rt` was understated by a factor of about 3.8 with 100 trials per cell and 3.5 to 3.6 with 10 (less at accuracies above .95, and, with an asymmetric start point in the 4-parameter model, more at the boundary nearer the start point, up to 5.7 for `zr` between .3 and .7, and less at the other), their correlation of about 0.7 was ignored, and posteriors came out too narrow. Both versions now use the exact first four cumulants of the first-passage time: `var_rt` follows a Gamma matched to the true mean and variance of a sample variance, and `mean_rt` is normal conditional on it, reproducing `Cov(mean_rt, var_rt)` exactly. The terms reduce to the previous ones when the third and fourth cumulants vanish. `rezdm()` samples from the same distribution and no longer truncates `mean_rt` at `ndt`, which `dezdm()` never assumed; when a summary rests on few responses (a handful of trials in the 3-parameter model, or a rarely reached boundary in the 4-parameter model, whatever `n_trials` is) it can therefore return `mean_rt <= 0`, which `bmm()` rejects; drop those rows before fitting. Existing **ezdm** fits are not reproduced by this version: subject-level credible intervals for `bound` widen, by 20% to 75% in our simulations, while those for the accuracy combination of drift and bound barely move. A fit cached with `bmm(file = )` is reused as-is unless `file_refit = "on_change"` or `"always"`. The exact cumulants cost more to evaluate than the two moments they replace; `?ezdm_dist` gives the measured cost (#407).
* Fix `ezdm` moments returning `NaN` at large `|drift|`. The 4-parameter moments used `cosh(x)/sinh(x)`, which is `Inf/Inf` above `x = 710`, so every 4-parameter moment computed in R (`dezdm()`, `rezdm()`, and through them `log_lik()` and `pp_check()`) was `NaN` from roughly `drift * bound / s^2 > 710`; the proportion correct used differences of `exp(2 k z)` and was `NaN` from about `|drift| * bound / s^2 > 710` at negative drift. Both are now written in forms that stay finite over any reachable drift, and the moments are computed from `drift^2 / s^4` throughout. Because they depend on the drift rate only through its square, the soft absolute value `sqrt(drift^2 + 0.01^2)` (which biased `drift^2` by `1e-4`) and the separate near-zero-drift formulas are both gone, removing a jump in the likelihood at `|drift| = 1e-6`, where the code switched between the two. Both likelihoods and `dezdm()` now take the binomial on the logit scale (`drift * bound / s^2` with a symmetric start point), computed without taking the logarithm of a probability that has rounded to 0 or 1. The density therefore stays finite where the proportion correct rounds to 1, and the 4-parameter density is unchanged when the sign of drift, `zr` and the two boundaries are mirrored. In both likelihoods the proportion correct now carries its gradient through `drift = 0`; the old code used a constant (0.5, or `zr`) below `|drift| = 1e-6`, so the response counts said nothing about the direction of drift there. `dezdm()` now refuses counts that are not whole numbers (#407).
