...
### New models
* Add **Multinomial Processing Tree models** (`mpt`) for tasks with categorical responses generated by discrete cognitive processing states. Models are specified as trees via `mpt_tree()`; equality constraints follow from shared parameter names. The implementation supports multiple experimental conditions (trees), logit and probit links, design-fixed covariates inside branch expressions, simplex-constrained parameter groups via stick-breaking, and non-linear (e.g., time-varying) parameter formulas. Includes distribution functions `dmpt()` and `rmpt()`, import functions `mpt_from_string()` (MPTinR-style model definitions) and `mpt_from_eqn()` (EQN files, with automatic sanitization of parameter names and a reported renaming map), a `plot()` method that draws the processing-tree diagrams implied by the branch expressions, and an online article introducing the model (#255).
