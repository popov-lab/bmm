...
# Default: the RT distribution
pp_check(fit_fixed_zr)

# The response distribution (choice proportions)
pp_check(fit_fixed_zr, resp_var = "response")

# RTs signed by the response (lower-boundary RTs negative), so the
# speed *and* frequency of errors are checked in one display
pp_check(fit_fixed_zr, resp_var = "signed_rt")

# All checks as a panel, drawn from one shared joint simulation
pp_check(fit_fixed_zr, resp_var = "all")

# Facet any check by a predictor of the fitted model
pp_check(fit_instruction, resp_var = "signed_rt", group = "instruction")
