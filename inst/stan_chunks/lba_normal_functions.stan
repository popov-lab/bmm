...
real lba_normal_single_lpdf(real t, real v, real b, real A, real s) {
  ...
  return lba_log_clip(M) - log(A) - log_denom;
}
...
real lba_normal_single_lccdf(real t, real v, real b, real A, real s) {
  ...
  real log_denom = std_normal_lcdf(v / s | );
  // Phi(-v/s) = 1 - Phi(v/s) = -expm1(log_denom); reuses log_denom (one fewer lcdf)
  real corrected_surv_num = surv_num - A * (-expm1(log_denom));
  return lba_log_clip(corrected_surv_num) - log(A) - log_denom;
}
